...
exports.getAllDocs = async function (req, res) {
  try {
    ...
    let viewersDocs = allDocs.filter((doc) => {
      if (doc.viewers.includes(req.user._id)) {
        return true;
      }
      return false;
    });

    const docs = ownerDocs.concat(collabDocs).concat(viewersDocs);
...
exports.searchDocsByName = async function (req, res) {
  const { name } = req.body;
  const regex = `${name}`;
  try {
    const docs = await DocModel.find(
      { name: new RegExp(regex) },
      "name owner collaborators viewers"
    );
